...
tableextension 70114 "COL Sales Shipment Header" extends "Sales Shipment Header"
{
    fields
    {
        ...
        field(70114; "COL No. of Packages"; Integer)
        {
            ...
            CalcFormula = count("COL Posted Package Dimension" where("Table Id" = const(Database::"Sales Shipment Header"), "Document No." = field("No.")));
        }
#pragma warning disable AA0232
        field(70115; "COL Total Gross Weight"; Decimal)
        {
            ...
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = sum("COL Posted Package Dimension"."Gross Shipment Weight" where("Table Id" = const(Database::"Sales Shipment Header"), "Document No." = field("No.")));
        }
#pragma warning restore AA0232
    ...
    begin
        ...
        if SalesShipmentLine.FindSet() then
            repeat
                LineFound := true;
                COLGetIfHigherPriority(LicenseRequired, SalesShipmentLine);
    ...
    local procedure COLGetIfHigherPriority(var LicenseRequired: Enum "COL Item Export Classification"; var SalesShipmentLine: Record "Sales Shipment Line")
    begin
        if LicenseRequired = Enum::"COL Item Export Classification"::Unknown then begin
            LicenseRequired := SalesShipmentLine."COL Export Classification Code";
            exit;
        end;

        if SalesShipmentLine."COL Export Classification Code" = Enum::"COL Item Export Classification"::Unknown then
            exit;

        if LicenseRequired.AsInteger() > SalesShipmentLine."COL Export Classification Code".AsInteger() then
            LicenseRequired := SalesShipmentLine."COL Export Classification Code";
    end;
}